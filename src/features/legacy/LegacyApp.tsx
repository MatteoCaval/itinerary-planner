...
import { DaySection, Location } from '../../types';
...
import { DEFAULT_CATEGORY, DEFAULT_SECTION } from '../../constants/daySection';
import { useAppModals } from '../../hooks/useAppModals';
import { PlannerPane, PlannerPaneProps } from '../planner/PlannerPane';
import { MapPane, MapPaneProps } from '../map/MapPane';
import { DesktopInspectorPane } from '../inspector/InspectorPane';
import { useTripActions } from '../controllers/useTripActions';
import { useSelectionFlow } from '../controllers/useSelectionFlow';
import { useSubItineraryActions } from '../controllers/useSubItineraryActions';
import { TripActionDialogs } from '../trips/TripActionDialogs';
...
function LegacyAppContent() {
  ...
  const {
    ...
    editingRoute,
    setEditingRoute,
    editingDayAssignment,
    setEditingDayAssignment,
    inspectorMode,
    setInspectorMode,
    pendingAddToDay,
    setPendingAddToDay,
  ...
  const [tripDialog, setTripDialog] = useState<'none' | 'create' | 'rename' | 'delete' | 'signout'>(
    'none',
  );
  ...
  const handleAddLocation = async (
    ...
    let assignedDayId =
      targetDayId || pendingAddToDay?.dayId || (days.length > 0 ? days[0].id : undefined);
  ...
  useEffect(() => {
    ...
    const loadSampleDemo = async () => {
      try {
        const response = await fetch(`${import.meta.env.BASE_URL}sample-trip.json`, {
          cache: 'no-store',
        });
        if (!response.ok) {
  ...
  const selectedLocation = useMemo(() => {
    const top = locations.find((location) => location.id === selectedLocationId);
    if (top) return top;
    for (const location of locations) {
      if (location.subLocations) {
        const sub = location.subLocations.find((item) => item.id === selectedLocationId);
        if (sub) return sub;
      }
    }
    return null;
  ...
  const handleSearch = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (results.length > 0) {
        await handleAddLocationWrapped(
      ...
      notifications.show({
        color: 'red',
        title: 'Search failed',
        message: 'Could not complete place search.',
      });
    } finally {
  ...
  const findLocationById = (id: string): Location | null => {
    const top = locations.find((location) => location.id === id);
    if (top) return top;
    for (const location of locations) {
      if (location.subLocations) {
        const sub = location.subLocations.find((item) => item.id === id);
        if (sub) return sub;
      }
    }
    return null;
  ...
  const plannerPaneProps: PlannerPaneProps = {
    ...
    exitSubItinerary: handleExitSubItinerary,
    activeDays,
    sidebarLocations,
    handleSubReorder,
    handleSubRemove,
  ...
  return (
    <>
      <Box className="app-root-shell">
        <Box h={60} w="100%" className="app-topbar">
          <AppHeader
            ...
            onSignOut={() => setTripDialog('signout')}
      ...
      >
        {mobileSheetView === 'details' && selectedLocation ? (
          <AppErrorBoundary
            ...
            message="The detail panel crashed. You can retry or reload the app."
          >
            <LocationDetailPanel
              {...detailPanelProps}
      ...
      <TripActionDialogs
        ...
        onCloseCreate={() => setTripDialog('none')}
        onCloseRename={() => setTripDialog('none')}
        onCloseDelete={() => setTripDialog('none')}
        onCloseSignOut={() => setTripDialog('none')}
        onConfirmCreate={() => {
      ...
      <AppErrorBoundary
        ...
        message="Cloud sync crashed. You can retry or reload the app."
      >
        <CloudSyncModal
          show={showCloudModal}
          onClose={() => setShowCloudModal(false)}
          getData={getExportData}
          onLoadData={loadFromData}
        />
      </AppErrorBoundary>
      ...
      <AppErrorBoundary
        ...
        message="The AI planner crashed. You can retry or reload the app."
      >
        <AIPlannerModal
          show={showAIModal}
          ...
          onApplyItinerary={(nextLocations, nextRoutes, updatedDays) => {
            const result = loadFromData({
              days: updatedDays || days,
            ...
            if (!result.success) {
              notifications.show({
                ...
                message: result.error || 'Generated itinerary was invalid.',
              });
            }
          }}
        />
      ...
      <RouteEditor
        ...
        route={
          routes.find(
            (route) =>
              (route.fromLocationId === editingRoute?.fromId &&
                route.toLocationId === editingRoute?.toId) ||
              (route.fromLocationId === editingRoute?.toId &&
                route.toLocationId === editingRoute?.fromId),
          ) ||
          (editingRoute
            ? {
                id: uuidv4(),
